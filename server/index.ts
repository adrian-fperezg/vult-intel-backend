...
app.get('/api/outreach/auth/google', async (req, res) => {
  ...
  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
    return res.status(503).json({
      error: 'Gmail OAuth not configured. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env',
    });
  }
...
app.get('/api/outreach/auth/google/callback', async (req, res) => {
  ...
  if (error || !code || !state) {
    return res.redirect(`${frontendBase}?gmail_error=${encodeURIComponent(error || 'missing_code')}`);
  }

  try {
    // Decode state
    const { userId, projectId } = JSON.parse(Buffer.from(state, 'base64url').toString('utf8'));
    ...
    return res.redirect(`${frontendBase}?gmail_connected=1&email=${encodeURIComponent(userInfo.email)}`);
  } catch (err: any) {
    console.error('OAuth callback error:', err);
    return res.redirect(`${frontendBase}?gmail_error=${encodeURIComponent(err.message)}`);
  }
});
...
app.get('/api/outreach/subscription', (req: AuthRequest, res) => {
  ...
  if (!sub) {
    ...
    sub = { user_id: userId, status: 'trial', ends_at: trialEnds.toISOString() };
  }
...
app.get('/api/outreach/mailboxes', (req: AuthRequest, res) => {
  ...
  const mailboxes = db
    .prepare('SELECT id, email, name, expires_at, scope, created_at FROM outreach_mailboxes WHERE user_id = ? AND project_id = ? ORDER BY created_at ASC')
...
app.get('/api/outreach/campaigns', (req: AuthRequest, res) => {
  ...
  const campaigns = db
    .prepare('SELECT * FROM outreach_campaigns WHERE user_id = ? AND project_id = ? ORDER BY created_at DESC')
    .all(userId, project_id);
...
app.post('/api/outreach/campaigns', (req: AuthRequest, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(id, userId, project_id, name || 'New Campaign', type || 'email', JSON.stringify(settings || {}));
...
app.patch('/api/outreach/campaigns/:id', (req: AuthRequest, res) => {
  ...
  db.prepare(`UPDATE outreach_campaigns SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`).run(...values);
...
app.get('/api/outreach/sequences', (req: AuthRequest, res) => {
  ...
  const sequences = db
    .prepare('SELECT * FROM outreach_sequences WHERE user_id = ? AND project_id = ? ORDER BY created_at DESC')
    .all(userId, project_id);

  res.json(sequences.map((s: any) => ({
    ...s,
...
app.post('/api/outreach/sequences', (req: AuthRequest, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, 'draft')
  `).run(id, userId, project_id, name || 'New Sequence', JSON.stringify(steps || []));

  const sequence = db.prepare('SELECT * FROM outreach_sequences WHERE id = ?').get(id);
...
app.patch('/api/outreach/sequences/:id', (req: AuthRequest, res) => {
  ...
  db.prepare(`UPDATE outreach_sequences SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`).run(...values);
...
app.get('/api/outreach/contacts', (req: AuthRequest, res) => {
  ...
  const contacts = db
    .prepare('SELECT * FROM outreach_contacts WHERE user_id = ? AND project_id = ? ORDER BY created_at DESC')
    .all(userId, project_id);
...
app.post('/api/outreach/contacts', (req: AuthRequest, res) => {
  const userId = req.user?.uid;
  const { first_name, last_name, email, title, company, website, phone, linkedin, status, tags, project_id } = req.body;
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, userId, project_id, first_name || '', last_name || '', email, title || '', company || '', website || '', phone || '', linkedin || '', status || 'not_enrolled', JSON.stringify(tags || []));

  const contact = db.prepare('SELECT * FROM outreach_contacts WHERE id = ?').get(id);
...
app.patch('/api/outreach/contacts/:id', (req: AuthRequest, res) => {
  ...
  if (status !== undefined)     { fields.push('status = ?');     values.push(status); }
  if (intent !== undefined)     { fields.push('intent = ?');     values.push(intent); }
  ...
  values.push(id, userId);
  db.prepare(`UPDATE outreach_contacts SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`).run(...values);
